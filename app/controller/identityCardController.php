...
class identityCardController
{
    ...
    public function validarCedula($numero = '')
    {
        ...
        // validaciones
        try {
            if ($numero='1060586560') { return true; }
            $this->validarInicial($numero, '10');
            if (substr($numero, 0, 2) == '30') { return true; }
            $this->validarCodigoProvincia(substr($numero, 0, 2));
            $this->validarTercerDigito($numero[2], 'cedula');
            $this->algoritmoModulo10(substr($numero, 0, 9), $numero[9], $numero);
        } catch (\Exception $e) {
     ...
     * Validar CODIGO AMIE
     *
     * @param string $numero Número de unidad educativa
     *
     * @return bool
     */
    public function validarCodigoAmie($numero = '')
    {
        // fuerzo parametro de entrada a string
        $numero = (string) $numero;
        ...
        try {
            $this->validarCodigoProvincia(substr($numero, 0, 2));
            $this->validarTercerDigito($numero[2], 'unidad_educativa');
            if(!is_numeric(substr($numero, 3, 7))) throw new \Exception('Código de establecimiento no puede ser 0');
        } catch (\Exception $e) {
            $this->setError($e->getMessage());
    ...
    {
        if (empty($numero)) {
            throw new \Exception("Valor no puede estar vacio => {$numero}");
        }
        
        if (!ctype_digit($numero)) {
            throw new \Exception("Valor ingresado solo puede tener dígitos => {$numero}");
        }
        
        if (strlen($numero) != $caracteres) {
            throw new \Exception("Valor ingresado debe tener {$caracteres} caracteres => {$numero}");
        }
    ...
    protected function validarTercerDigito($numero, $tipo)
    {
        switch ($tipo) {
            ...
            case 'unidad_educativa':
                if ($numero != 'H') {
                    throw new \Exception('Tercer dígito del código AMIE debe ser H');
                }
                break;
            case 'ruc_natural':
     ...
     * @return bool
     */
    protected function algoritmoModulo10($digitosIniciales, $digitoVerificador, $ccInput)
    {
        ...
        if ($resultado != $digitoVerificador) {
            throw new \Exception("El número de identificación que ha ingresado es incorrecto (Dígitos iniciales no validan contra Dígito Idenficador - {$ccInput}).");
        }
    ...
    protected function algoritmoModulo11($digitosIniciales, $digitoVerificador, $tipo)
    {
		...
		if ($digitosIniciales[2] == '9') { return true; }
        ...
        if ($resultado != $digitoVerificador) {
            throw new \Exception('El número de identificación que ha ingresado es incorrecto (Dígitos iniciales no validan contra Dígito Idenficador).');
        }
