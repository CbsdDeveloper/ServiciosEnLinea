 ...
 *
 */
class arrearModel extends breakModel {
	
	/*
	 * VARIABLES GLOBALES
	 */
	private $entity='atrasos';
	private $insert=0;
	private $update=0;
	...
	private function processList($pathFile){
		...
		$data=$myExcel->getCellValue($this->post,$pathFile,'atrasoslista');
	...
	public function uploadList(){
		// SUBIR ARCHIVO
		$config=$this->uploaderMng->config['entityConfig']['atrasoslista'];
		...
		$imgNew=$sql['mensaje'];
		// OBTENER ARCHIVO
		$pathFile=$this->uploaderMng->pathFile."{$config['folder']}/{$imgNew['atraso_name']}";
