...
class modificationModel extends vbpModel {
	
	/*
	 * VARIABLES DE ENTORNO
	 */
	private $entity='vbp_modificaciones';
	...
	private function requestByCode($code){
		...
		$str=$this->db->selectFromView("vw_{$this->entity}","WHERE UPPER(modificacion_codigo)=UPPER('{$code}')");
		// VALIDAR CONSULTA
		if($this->db->numRows($str)>0){
			...
			if(in_array($row['modificacion_estado'],['PENDIENTE'])){
				...
				$row['modificacion_estado']='INGRESADO';
				// MODAL A DESPLEGAR
				$row['modal']='Input';
				$row['tb']=$this->entity;
				$row['REQUIRED_FORM']=$this->testCollectionRequestSpecies($this->entity);
			}elseif(in_array($row['modificacion_estado'],['INGRESADO'])){
				// VALIDAR ROL DE INGRESO DE SOLICITUD
				if(!in_array(6043,$this->getAccesRol())) $this->getJSON("No tiene permisos para asignar personal para la revisión!");
				// GENERAR DATOS PARA ASIGNAR UN INSPECTOR
				$row['modificacion_estado']='ASIGNADO';
				// MODAL A DESPLEGAR
				$row['modal']='modificacionesinspector';
				$row['tb']=$this->entity;
			}elseif(in_array($row['modificacion_estado'],['ASIGNADO','REVISION','CORRECCION'])){
				// VALIDAR ROL DE INGRESO DE SOLICITUD
				if(!in_array(6044,$this->getAccesRol())) $this->getJSON("No tiene permisos para registrar la revisión de este proyecto!");
				// MODAL A DESPLEGAR
				$row['modal']=$this->entity;
				$row['tb']=$this->entity;
			}elseif(in_array($row['modificacion_estado'],['REVISADO'])){
				// VALIDAR PERMISO PARA GENERAR ORDEN DE COBRO
				if(!in_array(6045,$this->getAccesRol())) $this->getJSON("No cuenta con los privilegios para generar la orden de cobro de la solicitud <b>#$code</b>, por favor direccione el trámite con quien corresponda para que genere la orden de cobro!");
				// DATOS PARA GENERAR ORDEN DE COBRO
				$row['entity']=$this->entity;
				$row['entityId']=$row['modificacion_id'];
				// MODAL A DESPLEGAR
				$row['modal']='Generarordenescobro';
				$row['edit']=false;
			}elseif(in_array($row['modificacion_estado'],['ORDEN DE COBRO GENERADA'])){
				$this->getJSON("La solicitud <b>#$code</b> de PERMISO DE OCUPACIÓN Y modificacion se encuentra pendiente de pago!");
			}elseif(in_array($row['vbp_estado'],['PAGADO'])){
				$this->getJSON("La solicitud <b>#$code</b> de PERMISO DE OCUPACIÓN Y modificacion ya se encuentra pagada, por favor dirija el trámite a quien corresponda autorizar el respectivo permiso!");
			}elseif(in_array($row['modificacion_estado'],['APROBADO'])){
				$this->getJSON("La solicitud <b>#$code</b> de PERMISO DE OCUPACIÓN Y modificacion ya ha sido revisado y aprobado!");
			}else{
	...
	public function requestEntity(){
		...
		// INSPECTORES POR ID
		elseif(isset($this->post['type']) && $this->post['type']=='getInspectors') $json=$this->getInspectorsByCode($this->post['modificacion_id'],$this->entity);
		// LISTADO DE LAYERS
	...
	public function insertEntity(){
		...
		// SET MAPA DE COORDENADAS - VISTO BUENO DE PLANOS
		$this->setMapCoordinates('vbp',$this->post['fk_vbp_id'],$this->post);
		// SET GEOJSON
		$this->setGeoJSON('vbp',$this->post['fk_vbp_id'],$this->post['coordenada_marks']);
		...
		$this->post['modificacion_proyectista_id']=$vbp['proyectista_id'];
		$this->post['modificacion_responsable_mt_rpmci']=$vbp['responsable_mt_rpmci'];
		
		// INSERTAR RESPONSABLE DE PREVENCION - SOLICITUD
		$this->post['jtp_solicitud']=$this->varGlobal['RUPIF_PPERSONAL_ID'];
		
		// VALIDAR DATOS DE FACTURACIÓN
		if($this->post['modificacion_facturacion']=='MISMA') $this->post['modificacion_facturacion_id']=$vbp['fk_entidad_id'];
		else{
			if(isset($this->post['billing']['entidad_id']) && $this->post['billing']['entidad_id']>0) $this->post['modificacion_facturacion_id']=$this->post['billing']['entidad_id'];
	...
	public function updateEntity(){
		...
		// VALIDAR DATOS DE FACTURACIÓN
		if($this->post['modificacion_facturacion']=='MISMA') $this->post['modificacion_facturacion_id']=$this->post['fk_entidad_id'];
		else{
			if(isset($this->post['billing']['entidad_id']) && $this->post['billing']['entidad_id']>0) $this->post['modificacion_facturacion_id']=$this->post['billing']['entidad_id'];
			else $this->getJSON("No se ha encontrado los datos para facturación, por favor asegúrese de ingresar bien los datos!.");
		}
		...
		$oldData=$this->db->viewById($this->post['modificacion_id'],$this->entity);
		// VALIDAR POSIBLES ESTADOS DE REGISTRO
		if($this->post['modificacion_estado']=='INGRESADO'){
			// VALIDAR NÚMERO DE SOLICITUD
			if($this->testCollectionRequestSpecies($this->entity)=='SI') $this->post['modificacion_solicitud']=$this->testRequestNumber($this->entity,$this->post['modificacion_solicitud'],$this->post['modificacion_id']);
		}elseif($this->post['modificacion_estado']=='APROBADO'){
			
			// INSERTAR RESPONSABLE DE PREVENCION - APRUEBA
			$this->post['jtp_aprueba']=$this->varGlobal['RUPIF_PPERSONAL_ID'];
			
			// GENERAR NÚMERO DE SERIE
			if($this->post['modificacion_serie']<1) $this->post['modificacion_serie']=$this->db->getNextSerie($this->entity);
		}else{
			...
			if(isset($this->post['asignInspector']) && $this->post['asignInspector']){
				...
				$this->setPersonal($this->entity,$this->post['modificacion_id'],$this->post['selected']);
			}
		}
		...
		$sql['data']=array('id'=>$this->post['modificacion_id']);
	...
	/*
	 * IMPRIMIR SOLICITUD CBSD
	 */
	public function printDetail($pdfCtrl,$config){
		
		// GENERAR STRING DE CONSULTA
		$str=$this->db->selectFromView("vw_{$this->entity}","WHERE modificacion_id={$this->get['id']}");
		...
		if($this->db->numRows($str)>0){
			...
			$row['MODIFICACIONES_SOLICITUD']=$this->varGlobal["MODIFICACIONES_SOLICITUD_{$templateKey}"];
			...
			// GET BARCODE
			$pdfCtrl->setBQCode('barcode',$row['modificacion_codigo']);
	...
	public function printById($pdfCtrl,$id,$config){
		// STRING DE CONSULTA
		$str=$this->db->selectFromView("vw_{$this->entity}","WHERE modificacion_id={$id}");
		// VALIDAR SI EXISTE PERMISO DE FUNCIONAMIENTO
		if($this->db->numRows($str)>0){
			// EMPEZAR TRANSACCIÓN
			$this->db->begin();
			...
			if($row['modificacion_estado']=='REVISADO' && isset($this->get['exemptPayment'])){
				...
				$row['modificacion_estado']='APROBADO';
			}
			
			// VALIDAR ESTADO DE REGISTRO
			if(in_array($row['modificacion_estado'],['PENDIENTE','INGRESADO'])) return $pdfCtrl->printCustomReport('<li>El PERMISO DE OCUPACIÓN Y HABITABILIDAD no ha sido aprobado</li>',$row['responsable_firma_nombre'],$this->varGlobal['EMPTY_REPORT_DESCRIPTION']);
			
			// GENERAR NUMERO DE TRÁMITE - EDICIÓN DE 0 POR DEFECTO
			if($row['modificacion_estado']=='PAGADO'){
				// TRANSICIÓN DE ESTADO
				$row['modificacion_estado']='APROBADO';
				$row['modificacion_aprobado']=$this->getFecha('dateTime');
				$row['modificacion_anio']=$this->setFormatDate($row['modificacion_aprobado'],'Y');
				...
				$row['modificacion_aprueba']=$this->db->findOne($strAprueba)['ppersonal_id'];
				// GENERAR NÚMERO DE SERIE DE DESPACHADOS
				if($row['modificacion_serie']==0) $row['modificacion_serie']=$this->db->getNextSerie($this->entity);
				$sql=$this->db->executeTested($this->db->getSQLUpdate($row,$this->entity));
			}
			
			// NOMBRE DE RESPONSABLE QUE APRUEBA EL VBP
			$row['modificacion_aprueba_nombre']=$this->db->findOne($this->db->selectFromView('vw_personal',"WHERE ppersonal_id={$row['modificacion_aprueba']}"))['personal_nombre'];
			// PARSE FECHAS
			$row['vbp_aprobado']=$this->setFormatDate($row['vbp_aprobado'],'complete');
			$row['modificacion_fecha']=$this->setFormatDate($row['modificacion_fecha'],'complete');
			$row['modificacion_revisado']=$this->setFormatDate($row['modificacion_revisado'],'complete');
			$row['modificacion_aprobado']=$this->setFormatDate($row['modificacion_aprobado'],'complete');
			// NUMERO DE PERMISO
			$row['modificacion_serie']=str_pad($row['modificacion_serie'],3,"0",STR_PAD_LEFT);
			...
			// DATOS DE PAGO
			$billing=$this->db->findOne($this->db->selectFromView('ordenescobro',"WHERE orden_entidad='{$this->entity}' AND orden_entidad_id={$id}"));
