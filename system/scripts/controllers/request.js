...
.controller('submitCtrl',['$scope','myResource','findEntityService',function($scope,myResource,findEntityService) {
	...
	var uri=myResource.getTempData('uri'); // nombre de tabla para guardar Ej. producto
	...
	$scope.JSONEditorLoaded=function(jsonEditor){jsonEditor.expandAll();};
	$scope.JSONOptions={mode:'tree'};
	// ********************************** MODALES ************************************************
	// ******************************** MODALES AUXILIARES
	...
	$scope.submitRequestForm=function(){
		if($scope.toogleEditMode){
			myResource.requestData(tb).update($scope[frm],function(json){
				myResource.myDialog.showNotify(json);
				if(json.estado===true)myResource.myDialog.closeDialog();
				// DATOS DE RETORNO DE BACKEND
				if(myResource.testNull(json.data)) $scope.jsonResource=json.data;
			...
			myResource.requestData(tb).save($scope[frm],function(json){
				if(json.estado===true){
					$scope[frm]={};
					myResource.myDialog.closeDialog();
				}	myResource.myDialog.showNotify(json);
	...
	// ENVIO DE FORMULARIO CON WS NODE
	$scope.submitRequested=function(){
		$scope[frm].sessionId=account.session.usuario_id;
		if($scope.toogleEditMode){
			myResource.requestData(uri).update($scope[frm],function(json){
				myResource.myDialog.showNotify(json);
				if(json.estado===true)myResource.myDialog.closeDialog();
				// DATOS DE RETORNO DE BACKEND
				if(myResource.testNull(json.data)) $scope.jsonResource=json.data;
			...
			myResource.requestData(uri).save($scope[frm],function(json){
				if(json.estado===true){
					$scope[frm]={};
					myResource.myDialog.closeDialog();
				}	myResource.myDialog.showNotify(json);
