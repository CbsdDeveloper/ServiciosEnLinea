...
(function() {
  ...
  /**
   * Sets up the controllers's reference to ngModelController.
...
(function() {
   ...
   * the rest of the calendar controllers logic.
   */
...
(function() {
  ...
  /** Gets the controllers instance for the calendar in the floating pane. */
...
function GridListDirective($interpolate, $mdConstant, $mdGridLayout, $mdMedia) {
  ...
  function postLink(scope, element, attrs, ctrl) {
    ...
    // Provide the controllers with a way to trigger layouts.
...
function GridTileDirective($mdMedia) {
  return {
    ...
    transclude: true,
    scope: {},
    // Simple controllers that exposes attributes to the grid directive
...
function SelectDirective($mdSelect, $mdUtil, $mdTheming, $mdAria, $compile, $parse) {
  return {
    ...
    compile: compile,
    controller: function() {
    } // empty placeholder controllers to be initialized in link
  };
...
function SelectMenuDirective($parse, $mdUtil, $mdTheming) {
  ...
  function SelectMenuController($scope, $attrs, $element) {
    ...
    // and values matching every option's controllers.
    self.options = {};
...
function SidenavService($mdComponentRegistry, $q) {
  return function(handle) {
    ...
    // Lookup the controllers instance for the specified sidNav instance
    var self;
...
function SidenavDirective($mdMedia, $mdUtil, $mdConstant, $mdTheming, $animate, $compile, $parse, $log, $q, $document) {
  ...
  function postLink(scope, element, attr, sidenavCtrl) {
     ...
     * apply the CSS close transition... Then notify the controllers
 ...
 * @param {!VirtualRepeatContainerController} container The container's controllers.
...
function MdAutocompleteCtrl ($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
  ...
  /**
   * Initialize the controllers, setup watchers, gather elements
   */
  function init () {
  ...
  /**
   * Removes any events or leftover elements created by this controllers
   */
  function cleanup () {
  ...
  /**
   * Gathers all of the elements needed for this controllers
   */
 ...
 * bound to scope and controllers in {@code MdChipsDirective} as
 * {@code onAppend}. Due to the nature of directive scope bindings, the
 * controllers cannot know on its own/from the scope whether an expression was
 * actually provided.
 */
...
MdChipsCtrl.prototype.configureUserInput = function(inputElement) {
  ...
  // `.controllers` will look in the parent as well.
...
"use strict";
  ...
  function MdChips ($mdTheming, $mdUtil, $compile, $log, $timeout) {
    ...
    function compile(element, attr) {
      ...
      /**
       * Configures controllers and transcludes.
       */
      return function postLink(scope, element, attrs, controllers) {
        ...
        if (attr.ngModel) {
          ...
          // If an `md-on-append` attribute was set, tell the controllers to use the expression
          // when appending chips.
          if (attrs.mdOnAppend) mdChipsCtrl.useOnAppendExpression();

          // If an `md-on-remove` attribute was set, tell the controllers to use the expression
          // when removing chips.
          if (attrs.mdOnRemove) mdChipsCtrl.useOnRemoveExpression();

          // If an `md-on-select` attribute was set, tell the controllers to use the expression
...
function MenuDirective($mdUtil) {
  ...
  function link(scope, element, attrs, ctrls) {
    ...
    // Move everything into a md-menu-container and pass it to the controllers
...
function MdTabsController ($scope, $element, $window, $mdConstant, $mdTabInkRipple,
  ...
  /**
   * Perform initialization for the controllers, setup events and watcher(s)
   */
  function init () {
  ...
  /**
   * Remove any events defined by this controllers
   */
  function cleanup () {
    destroyed = true;
  ...
  /**
   * Gathers references to all of the DOM elements used by this controllers.
   * @returns {{}}
   */
