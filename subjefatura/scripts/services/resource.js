 ...
 */
app
.service('myResource',['$resource','$http','$filter','$sce','$state','$cookies','$httpParamSerializer','myDialog','upload','focus',
	function ($resource,$http,$filter,$sce,$state,$cookies,$httpParamSerializer,myDialog,upload,focus){
			...
			showNotify:function(json){// PRESENTAR NOTIFICACION
				...
				if(this.testNull(json.estado)){
					if(json.estado===true) growl.success(mensaje);
						...
						if(json.estado=='warning') growl.warning(mensaje);
						else if(json.estado=='info') growl.info(mensaje);
...
.factory('principal',['$q','$http','myResource',function($q,$http,myResource){
    ...
    return {
    	...
    	identity: function(force) {
    		...
    		$http({
    		      method: 'POST',
    		      url: host_name+sys_name+'/personaloperativo/REQUEST',
    		      data: {account:'subjefatura'}
    		   }).then(function(data){
    			// VALIDAR SESION
