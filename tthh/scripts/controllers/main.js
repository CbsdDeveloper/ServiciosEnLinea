...
.controller('mainCtrl',['$rootScope','$scope','myResource','mySession',function($rootScope,$scope,myResource,mySession){
	...
	$scope.dataEntity.boss=account.superior;
	$scope.entityInfo=account.info;
	
	// JSON CONFIG
	$scope.paramsConf=account.setting;
	$scope.availableModules=account.modules;
	// DATOS ESTADÍSTICOS
	...
	$scope.openModal=function(frm,row){
		...
		var ctrlTemp='submitCtrl';
		// VARIABLE PARA DIRECTORIO DE MODAL
		var pathEntity=(myResource.testNull(nativePathEntity[Tb]))?pathEntity=nativePathEntity[Tb]:pathEntity=pathModal+$scope.pathEntity;
	...
	$scope.deleteItem=function(frm,name,id){
		myResource.myDialog.showConfirm('Está seguro que desea eliminar "'+name+'" de los registros?',function(){
			...
			myResource.setData(tb,customPath[tb]).remove({id:id},function(json){
